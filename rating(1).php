...
$sql = "SELECT id, stars, comment, submitted_at FROM ratings ORDER BY submitted_at DESC";
$result = $conn->query($sql);
$ratingCount = $result ? $result->num_rows : 0;
?>
...
<head>
    ...
    <title>Customer Ratings - FALCONS MOVIES</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        body {
            ...
            padding: 32px 18px;
            background:
                radial-gradient(circle at top left, rgba(243, 182, 31, 0.2), transparent 28rem),
...
<div class="container">
    <header class="dashboard-header">
        <div>
            <span class="eyebrow">Admin Reviews</span>
            <h1>Customer Ratings</h1>
            <p class="header-copy">Review audience feedback, star ratings, and remove entries that no longer need to appear in the admin record.</p>
        </div>
        ...
        <section class="table-card">
            <div class="table-toolbar">
                <div>
                    ...
                    <p>Sorted by newest submission first.</p>
                </div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= intval($row["id"]) ?></td>
                            <td class="stars"><?= htmlspecialchars($row["stars"]) ?> / 5</td>
                            <td class="comment-cell"><?= nl2br(htmlspecialchars($row["comment"])) ?></td>
                            <td><?= htmlspecialchars($row["submitted_at"]) ?></td>
                            <td>
                                <a class="delete-btn" href="rating.php?delete_id=<?= intval($row["id"]) ?>" onclick="return confirm('Are you sure you want to delete this rating?');">Delete</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                    </tbody>
