...
$priceColumnCheck = mysqli_query($conn, "SHOW COLUMNS FROM upcoming_movies LIKE 'ticket_price'");
if ($priceColumnCheck && mysqli_num_rows($priceColumnCheck) === 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs
    if (empty($name) || !validatePhone($phone) || empty($selectedSeats) || !validateMovieName($movie) || !isset($showTimes[$time]) || !isset($theaters[$theaterCode])) {
        echo json_encode(['success' => false, 'error' => 'Missing or invalid data.']);
        exit;
    }

    // Validate seats
    if (!validateSeatNumbers($selectedSeats)) {
        echo json_encode(['success' => false, 'error' => 'Invalid seat selection.']);
        exit;
    }
    ...
    $bookedStmt = $conn->prepare("SELECT seats FROM bookings WHERE movie = ? AND time = ? AND theater = ?");
    if (!$bookedStmt) {
    ...
    while ($bookingRow = $bookedResult->fetch_assoc()) {
        foreach (explode(',', $bookingRow['seats']) as $seat) {
    ...
    if (array_intersect($selectedSeats, $alreadyBooked)) {
        echo json_encode(['success' => false, 'error' => 'One or more selected seats were just booked. Please choose again.']);
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => false, 'error' => 'Error saving booking.']);
    }
    $stmt->close();
    exit;
}
...
if (isset($_GET['id'])) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        header("Location: buy.php?id=" . $row['id']);
        exit;
    }
...
<head>
    ...
    <title>Seat Booking - <?= htmlspecialchars($movieName) ?></title>
    ...
    <style>
        body {
            ...
            background: linear-gradient(rgba(0, 0, 0, 0.85), rgba(0, 0, 0, 0.85)),
                url('https://images.unsplash.com/photo-1578863579390-9b3bd2170c2d?auto=format&fit=crop&w=1950&q=80') no-repeat center center fixed;
        ...
        .seats-container {
            ...
            border-radius: 12px;
            background: radial-gradient(circle at 50% 15%, rgba(255, 215, 0, 0.18), transparent 32%),
                linear-gradient(180deg, rgba(7, 19, 29, 0.96), rgba(0, 0, 0, 0.78));
            box-shadow: inset 0 0 45px rgba(0, 0, 0, 0.6);
        }

        .seats-container canvas {
            display: block;
            width: 100%;
            height: 100%;
        }
...
<body>
    <h1>Seat Booking</h1>
    <div class="movie-label">Movie: <?= htmlspecialchars($movieName) ?></div>
    <div class="movie-label">
        Seat Prices:
        <?php foreach ($seatSections as $index => $section): ?>
            <?= $index > 0 ? ' | ' : '' ?><?= htmlspecialchars($section['name']) ?> Rs.<?= number_format($section['price'], 2) ?>
        <?php endforeach; ?>
    </div>
    <div class="wrapper">
        <div class="showtime-row">
            ...
            <select id="theater-select">
                <?php foreach ($theaters as $value => $label): ?>
                    <option value="<?= htmlspecialchars($value) ?>" <?= $value === $selectedTheaterCode ? 'selected' : '' ?>>
            ...
            <select id="show-time">
                <?php foreach ($showTimes as $value => $label): ?>
                    <option value="<?= htmlspecialchars($value) ?>" <?= $value === $selectedShowTime ? 'selected' : '' ?>>
                        <?= htmlspecialchars($label) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="seats-container" id="seats-container">
            <div class="seat-loading" id="seat-loading">Loading 3D theater seats...</div>
        </div>
        <div class="seat-legend" aria-label="Seat legend">
            <?php foreach ($seatSections as $section): ?>
                <span class="legend-item">
                    <span class="legend-swatch" style="background: <?= htmlspecialchars($section['color']) ?>"></span>
    ...
    <script>
        window.theaterBookingConfig = {
            movieName: <?= json_encode($movieName) ?>,
            selectedTheater: <?= json_encode($selectedTheaterCode) ?>,
            seatPrice: <?= json_encode($ticketPrice) ?>,
            seatCount: <?= json_encode($seatCount) ?>,
            seatSections: <?= json_encode($seatSections) ?>
        };
    </script>
